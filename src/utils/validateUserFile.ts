...
const hasOwnPropertyCb = (obj: Map<unknown, unknown>) => {
 ...
 */
export const validateUserFile = async (file: DropzoneFile | undefined): Promise<ValidateUserFileReturnType> => {
    if (file == null) {
        return { isValid: false, userFile: null };
    }

    // Parse JSON file to JS object
    let fileJSON: Map<unknown, unknown>;
    try {
        const parsedFile = await parseJSONFile(file);
        fileJSON = parsedFile as Map<unknown, unknown>;
    } catch (error) {
    ...
    try {
        ...
        return { isValid: false, error: ErrorMessages.FILE_VALIDATION, userFile: null }
    }
};
